...
### 1. 动态业务表单配置
在 CRM、OA 等企业管理系统中，不同客户或部门需要不同的表单字段。通过 fastsun-form 的动态表单设计，无需修改代码即可在线配置表单布局、字段类型和验证规则，快速响应业务变化。

### 2. 工作流审批表单
在流程审批场景中（如请假申请、报销审批），表单内容随流程节点动态变化。结合工作流引擎，实现表单字段在审批过程中的只读、编辑、隐藏等权限控制。

### 3. 用户信息采集与注册
在用户注册、问卷调查、信息收集等场景中，需要灵活定义采集字段。通过表单模块的字段类型支持和验证规则引擎，快速搭建多形态的数据采集表单。

### 4. 低代码平台表单构建
在低代码开发平台中，通过可视化拖拽方式构建表单，自动生成前后端代码。表单模块提供标准化的表单存储结构和渲染引擎，支撑低代码平台的快速搭建。
...
fastsun:
  platform:
    form:
      cache-enabled: true    # 是否启用表单缓存，启用后可加速表单配置加载 <span class="config-required">(必需)</span>
      cache-expire: 3600     # 表单缓存过期时间（秒），默认为 1 小时
      max-fields: 100        # 单个表单允许的最大字段数，防止字段过多影响性能
```
...
| 被依赖 | 业务模块 | 各业务模块通过动态表单快速实现数据采集功能 |
