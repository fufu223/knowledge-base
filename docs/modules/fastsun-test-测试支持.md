...
- **单元测试**：对 Service、Mapper 等核心逻辑编写单元测试，保障代码质量
- **API 集成测试**：使用 TestRestTemplate 对 Controller 接口进行端到端测试
- **Mock 数据生成**：在测试环境中快速生成符合规范的测试数据，提升测试开发效率
- **CI 流水线集成**：测试脚本可嵌入 CI/CD 流水线，自动执行并输出测试报告
