...
在电商平台中，根据会员等级、订单金额、商品品类等条件动态计算折扣、优惠券和促销活动，无需修改代码即可灵活调整营销策略。

### 2. 风控规则判定

在支付交易、用户登录等场景中，根据行为特征（如 IP 地址、设备指纹、交易金额）动态判定风险等级，触发人工审核或自动拦截。

### 3. 审批流程路由

在业务流程中，根据表单字段值（如金额、部门、紧急程度）动态决定审批流程走向和审批人，实现灵活的流程路由控制。

### 4. 业务规则配置化

将频繁变化的业务逻辑（如税费计算、积分规则、计费策略）抽取为可配置规则，业务人员可通过界面直接调整规则，减少开发交付周期。
...
规则引擎的核心运行时配置，控制规则启用状态、执行性能限制和缓存策略。
...
fastsun:
  platform:
    ruleflow:
      # 是否启用
      enabled: true  # 是否启用规则引擎 <span class="config-required">(必需)</span>
      # 最大规则数
      max-rules: 1000  # 允许创建的最大规则数量，超限后不可新增
      # 规则超时时间（毫秒）
      timeout: 5000  # 单条规则执行超时时间，超时自动终止
      # 是否启用缓存
      cache-enabled: true  # 是否启用规则缓存，启用后可提升执行性能
```
...
| 依赖类型 | 模块 | 说明 |
|---------|------|------|
| 调用依赖 | fastsun-ucenter | 用户中心，规则执行需要获取用户信息和上下文数据 |
| 调用依赖 | fastsun-gateway | API 网关，网关路由转发规则引擎请求 |
| 调用依赖 | fastsun-oauth | 认证授权，规则引擎 API 调用需要 Token 鉴权 |
| 调用依赖 | fastsun-service | 服务管理，规则可关联服务进行调用 |
| 存储依赖 | MySQL | 存储规则定义、规则版本历史等数据
