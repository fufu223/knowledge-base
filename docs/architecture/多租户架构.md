...
flowchart LR
    Client[客户端请求] -->|携带租户标识| Gateway[Gateway 网关]
    Gateway -->|解析 Token 提取租户| Auth[认证服务]
    Auth -->|设置租户上下文| LocalTenantContext["TenantContext<br/>线程本地上下文"]
    LocalTenantContext -->|字段隔离模式| Hibernate["Hibernate 拦截器<br/>自动添加 tenant_code"]
    LocalTenantContext -->|数据库隔离模式| MTCP["MultiTenantConnectionProvider<br/>动态切换数据源"]
    Hibernate --> SharedDB[("共享数据库<br/>tenant_code 区分")]
    MTCP --> DB1[("租户 A<br/>独立数据库")]
    MTCP --> DB2[("租户 B<br/>独立数据库")]
    MTCP --> DB3[("租户 C<br/>独立数据库")]

    classDef client fill:#1e40af,color:#fff,stroke:#1e3a8a,stroke-width:2px,font-weight:bold
    classDef gw fill:#7c3aed,color:#fff,stroke:#6d28d9,stroke-width:2px,font-weight:bold
    classDef svc fill:#0d9488,color:#fff,stroke:#0f766e,stroke-width:2px,font-weight:bold
    classDef process fill:#c2410c,color:#fff,stroke:#9a3412,stroke-width:2px,font-weight:bold
    classDef db fill:#1e293b,color:#e2e8f0,stroke:#475569,stroke-width:2px,font-weight:bold

    class Client client
    class Gateway gw
    class Auth,LocalTenantContext svc
    class Hibernate,MTCP process
    class SharedDB,DB1,DB2,DB3 db
```
...
graph TD
    Tenant["租户 Tenant"]:::tenant
    EntA["企业 A<br/>Enterprise"]:::ent
    EntB["企业 B<br/>Enterprise"]:::ent
    EntC["企业 C<br/>Enterprise"]:::ent
    DeptA1["部门 A-1"]:::dept
    DeptA2["部门 A-2"]:::dept
    DeptB1["部门 B-1"]:::dept
    ...
    classDef tenant fill:#1e40af,color:#fff,stroke:#1e3a8a,stroke-width:3px,font-weight:bold
    classDef ent fill:#c2410c,color:#fff,stroke:#9a3412,stroke-width:2px,font-weight:bold
    classDef dept fill:#0d9488,color:#fff,stroke:#0f766e,stroke-width:1px
```
